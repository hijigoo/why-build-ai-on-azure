...
**Microsoft의 접근:** 이 5가지를 개별 도구의 조합이 아니라, 신뢰할 수 있는 하나의 플랫폼으로 풀어가는 것이 Microsoft의 방식입니다. **이어지는 [§3](#s3)에서 각 벽에 Azure가 어떻게 답하는지** 근거를 정리합니다.
...
## 일반론이 아니라 — 구체적으로 무엇이 다른가

"신뢰할 수 있다"는 말은 추상적입니다. Azure가 **구체적으로** 무엇이 다른지 4가지로 정리하면:

- **① 데이터는 당신의 것** — 고객 입력·출력을 파운데이션 모델 학습에 사용하지 않는 원칙, 한국 리전 우선 설계, 테넌트 격리. (데이터 상주·약관은 서비스·모델별 확인)
- **② 모델 선택의 자유** — **Microsoft Foundry 모델 카탈로그**에서 프론티어(OpenAI)·오픈소스·산업별 모델을 **운영 가능한 방식으로** 골라 바꿔 씁니다.
- **③ 이미 가진 스택 위에서** — Microsoft 365·GitHub·Microsoft Fabric은 *연동 가치*로 얹히고, 이 기준을 실제로 책임지는 **엔진은 Azure**입니다. 원본을 옮기지 않고 **연결**부터 시작합니다.
- **④ AI를 감싸는 보안·거버넌스** — **Microsoft Entra ID**(신원)·**Microsoft Purview**(데이터)·**Microsoft Defender**(위협)·**Microsoft Agent 365**(에이전트 통제 *[GA · 2026.5]*)가 AI 전 계층을 관통합니다.

> 핵심은 "**어디서 구축·운영하느냐**" — Azure는 이 기준들을 하나의 플랫폼에서 일관되게 제공합니다.
...
```mermaid
flowchart TD
    D["데이터<br/>Microsoft Fabric·OneLake"] --> M["의미·모델<br/>Microsoft Foundry·Azure AI Search"]
    M --> A["에이전트<br/>Microsoft Copilot Studio·Microsoft Foundry"]
    A --> E["경험<br/>Microsoft 365 Copilot·Microsoft Teams"]
...
> 🧭 이 스택이 실제로 어떤 계층으로 구성되는지는 Part 3에서 다룹니다. [**§6**](#s6)에서 필요한 요건을 먼저 정의하고, [**§7**](#s7)에서 Microsoft가 그 자리를 어떻게 채우는지 봅니다.
...
- **Microsoft 365** — 이미 일하는 공간(Microsoft Teams·Office)에 AI가 자연스럽게 스며듦
- **현업부터 개발자까지** — Low-Code(Microsoft Copilot Studio) ~ Pro-Code(Microsoft Foundry) 모두 지원

---

## AI를 어디서 구축·운영할 것인가 — 6가지 판단 기준

AI를 **어디서 구축·운영할지**는 감이 아니라 아래 기준으로 갈립니다. Azure는 이 기준들을 **하나의 플랫폼 안에서 일관되게** 제공합니다. (Microsoft 365 Copilot·GitHub·Microsoft Fabric은 *연동 가치*로 얹히고, 이 기준을 실제로 책임지는 **엔진은 Azure**입니다.)

| 판단 기준 | 왜 중요한가 (고객 관점) | Azure에서 어떻게 |
|-----------|------------------------|------------------|
| **데이터 위치·주권** | 데이터가 리전 밖으로 나가면 규제 리스크 | 한국 리전 우선 설계 · 서비스별 데이터 상주/리전 지원 확인 |
| **모델 운영** | 어떤 모델을 어떻게 안전하게 서빙할지 | Azure OpenAI · Microsoft Foundry에서 모델 선택·배포·버전 관리 |
| **보안 경계** | AI가 사내 데이터에 닿는 경로 통제 | Private Endpoint · VNet · 망분리 구성 |
| **네트워크 격리** | 프롬프트·응답·로그의 이동 통제 | Private Endpoint · VNet/방화벽 · 로그·트래픽 경로 통제(서비스별 지원 확인) |
| **거버넌스·감사** | 누가·무엇을·언제 접근했는지 추적 | Microsoft Entra ID · Microsoft Purview · Microsoft Defender 통합 |
| **비용 관측** | 토큰·라이선스·처리 비용의 가시성 | Cost Management · 사용량 모니터링 |

**즉 Why Azure?의 답은 개별 기능 하나하나가 아니라 — 데이터 주권·보안 경계·거버넌스·비용을 한곳에서 일관되게 통제할 수 있는 운영 관점에 있습니다.**

**실무에서 자주 나오는 상황별로 보면:**
- **규제·주권·최신 모델 운영이 중요한 경우:** 데이터 주권 요구가 크거나, 최신 프론티어 모델을 안전하게 운영하고 싶거나, Microsoft 365·GitHub·Microsoft Fabric을 이미 쓰고 있다면 Azure의 **연동 가치**가 특히 크게 살아납니다.
- **이미 데이터 플랫폼을 잘 쓰고 있는 경우:** 잘 돌아가는 데이터 웨어하우스·레이크가 있어도 **전면 교체가 필요하지 않습니다.** 원본은 그대로 두고 **AI가 활용할 수 있게 연결**하는 것부터 시작하면 됩니다. Azure는 기존 투자를 대체하는 것이 아니라 **그 위에 AI를 얹어 확장**하는 방식을 지원합니다. 목표는 벤더 교체가 아니라 **성과**이며, Azure는 두 경우 모두에 자연스럽게 맞춰집니다.
...
|---------|------------------|----------------|
| **① Agent 전사 Scale** | Agent Build + Agents + Experience | Microsoft Copilot Studio · Microsoft Foundry · Microsoft 365 Copilot + Microsoft Teams<br/>*(Microsoft Foundry→Microsoft 365/Microsoft Teams 게시는 Early Access Preview)* |
| **② 데이터 IQ Layer** | OneLake + Ontology | Microsoft Fabric · Fabric IQ + Work IQ + Foundry IQ *[Preview·확인 필요]* |
| **③ 보안·거버넌스** | Governance (전 계층 관통) | Microsoft Agent 365 *[GA · 2026.5]* · Microsoft Purview · Microsoft Entra ID · Microsoft Defender |
...
[§2](#s2)에서 본 다섯 가지 벽은 모델의 문제가 아니었습니다. 에이전트를 **만들고 · 쓰고 · 통제하는 기반**이 없어서 생긴 문제였습니다. 이를 요구사항의 언어로 바꾸면 여섯 가지가 됩니다.
