...
const VERSION = process.env.DECK_VERSION || "v10";
...
function part1() {
  ...
  s.addText("이미 Microsoft 365·GitHub·Microsoft Fabric을 쓰고 있다면, '지금' 시작하기 가장 가까운 곳은 그 위에 얹히는 Azure입니다.", { x: 0.6, y: 6.55, w: 12.15, h: 0.4, fontFace: FONT, fontSize: 12.5, bold: true, color: C.NAVY, align: "center" });
  footer(s, 5);
}
...
function part1b() {
  ...
  const trust = [
    ...
    ["Responsible AI", "설계부터 내장된 책임·안전 — Azure AI Content Safety · Microsoft Entra ID · Microsoft Purview · Microsoft Defender", IC.balance_w],
    ["End-to-End 완결 스택", "데이터(Microsoft Fabric·OneLake) → 에이전트(Microsoft Foundry) → 운영(Azure Monitor)까지 하나의 스택에서", IC.layers_w],
  ...
  s.addText("이 네 가지가 특히 강한 이유 — 이미 Microsoft 365·Microsoft Entra ID·GitHub·Microsoft Fabric을 쓰는 조직에겐, 새로 만드는 게 아니라 '쓰던 것' 위에 안전하게 얹힙니다.", { x: 0.6, y: 6.9, w: 12.15, h: 0.45, align: "center", fontFace: FONT, fontSize: 12.5, bold: true, color: C.NAVY });
  footer(s, 7);
}
...
function part1c() {
  ...
  const diff = [
    ...
    ["이미 가진 스택 위에서", "Microsoft 365 · GitHub · Microsoft Fabric은 연동 가치, 이 기준을 책임지는 엔진은 Azure — 옮기지 않고 연결부터", IC.plug_n],
    ["AI를 감싸는 보안·거버넌스", "Microsoft Entra ID(신원) · Microsoft Purview(데이터) · Microsoft Defender(위협) · Microsoft Agent 365(에이전트 통제, GA·2026.5)", IC.usershield_n],
  ];
  ...
  styledTable(s, [
    ...
    [{text:"신뢰성·안전",bold:true},"예측 가능하고 안전한 동작","Azure AI Content Safety · 평가 파이프라인"],
    [{text:"개인정보·보안",bold:true},"데이터 주권과 보호","고객 데이터 학습 미사용 · 리전 격리"],
    [{text:"투명성",bold:true},"설명 가능성","추적·관측(Observability)"],
    [{text:"책임성",bold:true},"거버넌스와 통제","Microsoft Entra ID · Microsoft Purview · Microsoft Defender 연동"],
  ...
  const crit = [
    ...
    ["보안 경계", "조직 보안·ID 안에서 도는가", "Microsoft Entra ID · Microsoft Purview · Microsoft Defender", IC.shield_n],
    ["모델 선택·운영", "최신 모델을 바꿔 쓸 수 있나", "Microsoft Foundry 모델 카탈로그", IC.brain_n],
    ["거버넌스", "누가·무엇을 쓰는지 통제되나", "Microsoft Agent 365 · Microsoft Purview 감사", IC.usershield_n],
    ["통합 가치", "이미 쓰는 시스템과 연결되나", "Microsoft 365 · GitHub · Microsoft Fabric 연동", IC.plug_n],
...
function part2() {
  ...
  styledTable(s, [
    ...
    [{text:"업무 지식",bold:true},"데이터·온톨로지 이해","Microsoft Fabric · OneLake · 3개 IQ"],
    [{text:"일하는 도구",bold:true},"시스템·API 연결","Microsoft Foundry Tools · 커넥터"],
    [{text:"판단·실행",bold:true},"모델·추론","Microsoft Foundry 모델 카탈로그"],
    [{text:"소속·권한",bold:true},"ID·접근 통제","Microsoft Entra Agent ID"],
    [{text:"관리·평가",bold:true},"성과·안전 관측","Microsoft Purview · Microsoft Defender · 평가"],
...
function part3a() {
  ...
  const svc = [
    ...
    ["Microsoft Copilot Studio", "현업·IT 에이전트", "코드 없이 업무 에이전트 제작", IC.comments_n, "ga"],
    ["Microsoft 365 · Microsoft Agent 365", "업무 현장 통합", "일하는 곳에서 바로 에이전트 활용", IC.windows_n, "ga"],
...
function part3b() {
  CUR_PART = 3;
  // Slide 16: ① Microsoft Fabric · OneLake
  let s = pptx.addSlide(); contentBG(s);
  ...
  // Slide 17: ② 3개 IQ + Azure AI Search  (convergence "equation")
  s = pptx.addSlide(); contentBG(s);
  header(s, "6-② 지능 레이어", "3개의 IQ가 합쳐져 '비즈니스를 아는 지능'이 됩니다");
  const iqs = [
    ["Fabric IQ", "데이터의 의미", "테이블·컬럼이 '무엇을 뜻하는지' 이해"],
    ["Work IQ", "일의 맥락", "Microsoft 365 속 업무·협업의 흐름을 이해"],
...
function part3c() {
  ...
  const four = [
    ...
    ["에이전트 개발", "Microsoft Foundry Agent Service로 에이전트를 만들고 배포", IC.robot_n],
    ["평가 · 관측", "품질·안전을 지속 측정하는 평가·Observability", IC.eye_n],
    ["안전 · 거버넌스", "Azure AI Content Safety로 안전 가드레일 내장", IC.shield_n],
  ];
  let x = 0.6, y = 2.7;
  four.forEach((f, i) => {
  ...
  // Slide 19: ④⑤⑥ Microsoft Copilot Studio / Microsoft 365·Microsoft Agent 365 / GitHub Copilot
  s = pptx.addSlide(); contentBG(s);
  header(s, "6-④⑤⑥ 만들고 · 일하고 · 개발한다", "Microsoft Copilot Studio · Microsoft 365 · GitHub Copilot");
  const three = [
    ["④ Microsoft Copilot Studio", "현업·IT를 위한 에이전트 빌더", ["코드 없이 업무 에이전트 제작", "Microsoft 365·Microsoft Teams로 바로 게시 (GA)", "Microsoft Foundry와 상호운용"], IC.comments_w, "ga"],
    ["⑤ Microsoft 365 · Microsoft Agent 365", "일하는 현장의 에이전트", ["Microsoft Teams·Outlook 등 업무 흐름 안에서", "Microsoft Agent 365로 에이전트를 관리·운영", "Microsoft Entra ID·Microsoft Purview로 통제"], IC.windows_w, "ga"],
    ["⑥ GitHub Copilot", "에이전트를 만드는 생산성", ["세계 최대 개발 생태계", "코딩 에이전트로 개발 가속", "Microsoft Foundry와 함께 앱·에이전트 구축"], IC.github_w, "ga"],
  ];
  let tx = 0.6;
  three.forEach(t => {
...
function part3d() {
  ...
  const layers = [
    ["Experience", "사용자·업무 경험", "Microsoft 365 · Microsoft Teams · Microsoft Copilot Studio 앱", IC.comments_w, C.MID],
    ["Agents", "에이전트", "Microsoft Foundry Agent Service · Copilot 에이전트", IC.robot_w, C.NAVY],
    ["Agent Factory", "에이전트 제작 계층", "Microsoft Copilot Studio(현업·IT) · Microsoft Foundry(개발자)", IC.cogs_w, C.MID],
  ...
  s.addText("Microsoft Entra ID · Microsoft Purview\nMicrosoft Defender · Azure Monitor", { x: 10.6, y: (gy0+gy1)/2 + 0.5, w: 1.7, h: 1.2, align: "center", fontFace: FONT, fontSize: 9.5, color: C.SKY, lineSpacingMultiple: 1.15 });
  // up arrow annotation
  ...
  const steps = [
    ...
    ["에이전트화", "Microsoft Foundry·Microsoft Copilot Studio로 제작", IC.robot_n],
...
function part3e() {
  ...
  styledTable(s, [
    ...
    [{text:"Experience",bold:true},"일하는 곳에서 바로 쓰는 경험","Microsoft 365 · Microsoft Teams · Microsoft Copilot Studio 앱"],
    [{text:"Agents",bold:true},"업무를 대신 실행하는 디지털 직원","Microsoft Foundry Agent Service · Copilot 에이전트"],
    [{text:"Agent Factory",bold:true},"현업·개발자 모두 에이전트 제작","Microsoft Copilot Studio · Microsoft Foundry"],
    [{text:"Ontology / IQ",bold:true},"비즈니스를 이해하는 지능","Fabric IQ · Work IQ · Foundry IQ · Azure AI Search"],
  ...
  const gov = [
    ["Microsoft Entra Agent ID", "ID · 접근", "에이전트에게도 신원과 권한을 부여·통제", IC.key_n],
    ["Microsoft Purview", "데이터 거버넌스", "민감정보 분류·데이터 정책·감사", IC.usershield_n],
    ["Microsoft Defender for Cloud/AI", "위협 보호", "AI 워크로드의 위협 탐지·대응", IC.shield_n],
...
function part3f() {
  ...
  styledTable(s, [
    ...
    [{text:"경험",bold:true},"Microsoft 365 · Microsoft Teams · Microsoft Copilot Studio",{text:"GA",color:C.GREEN,bold:true,align:"center"}],
    [{text:"에이전트 관리",bold:true},"Microsoft Agent 365",{text:"GA · 2026.5",color:C.GREEN,bold:true,align:"center"}],
    [{text:"에이전트 제작",bold:true},"Microsoft Copilot Studio · Microsoft Foundry",{text:"GA",color:C.GREEN,bold:true,align:"center"}],
    ...
    [{text:"거버넌스",bold:true},"Microsoft Entra ID · Microsoft Purview · Microsoft Defender · Azure Monitor",{text:"GA",color:C.GREEN,bold:true,align:"center"}],
...
function part4() {
  ...
  const per = [
    ...
    ["현업 담당자", "반복업무 자동화", "Microsoft Copilot Studio로 직접 만든 업무 에이전트", IC.users_n],
    ["개발자", "개발 생산성", "GitHub Copilot·Microsoft Foundry로 앱·에이전트 구축", IC.code_n],
    ["IT·보안", "통제 가능한 확산", "Microsoft Entra ID·Microsoft Purview로 안전하게 관리·감사", IC.usershield_n],
  ];
  let x = 0.6, y = 2.0;
  per.forEach((p, i) => {
  ...
  styledTable(s, [
    ...
    [{text:"1",align:"center"},"보고서·리서치 자동 작성","Microsoft Foundry · Azure AI Search · OneLake"],
    [{text:"2",align:"center"},"내부 지식 Q&A 어시스턴트","Microsoft Copilot Studio · Azure AI Search"],
    [{text:"3",align:"center"},"업무 프로세스 자동화","Microsoft Copilot Studio · Microsoft 365"],
    [{text:"4",align:"center"},"전사 에이전트 관리·운영","Microsoft Agent 365 · Microsoft Entra ID · Microsoft Purview"],
    [{text:"5",align:"center"},"데이터 분석·인사이트","Microsoft Fabric · Microsoft Foundry"],
    [{text:"6",align:"center"},"개발 가속·현대화","GitHub Copilot · Microsoft Foundry"],
...
function part4b() {
  ...
  const road = [
    ...
    ["4", "Govern", "상시", "Microsoft Entra ID·Microsoft Purview·Microsoft Defender로 통제·관측 정착"],
  ];
